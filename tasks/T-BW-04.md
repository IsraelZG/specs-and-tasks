...
# T-BW-04 · Bancada: Mídia real — upload/fontes sobre media plane + WebSeed
...
## 1. Objetivo
Substituir os placeholders de mídia do App.tsx (`midiaUpload` → "placeholder-infohash",
`midiaP2PMonitor` vazio, `midiaKillWebSeed` no-op) pelo media plane real: upload
chunking+cifra (T-801) → fontes/manifesto (T-802) → WebSeed do system-peer (T-804) e seeders
WebTorrent (T-803). Player/reidratação (`midiaPlay`) permanece FORA do escopo até T-805 —
deixar o botão desabilitado com tooltip citando T-805, não simular.

## 2. Contexto RAG (Spec-Driven Development)
- [ ] T-801/T-802 (chunking+manifesto)
- [ ] T-804 (WebSeed)
- [ ] T-803 (WebTorrent adapter — dep)
- [ ] T-807 (aba Mídia — UI pronta)
