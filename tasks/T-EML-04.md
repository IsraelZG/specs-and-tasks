---
id: T-EML-04
title: "UI de Email (caixa por thread, thread, compositor overlay, multi-conta)"
status: draft:placeholder
complexity: 4
target_agent: frontend_agent # perfis: devops_agent, logic_agent, crypto_agent, frontend_agent
reviewer_agent: agile_reviewer
execution_mode: sequential # parallel | sequential
dependencies: ["T-EML-01", "T-EML-02", "T-SHL-01"] # IDs de tarefas que bloqueiam esta
ui: true
...
# T-EML-04 · UI de Email (caixa por thread, thread, compositor overlay, multi-conta)
...
## 1. Objetivo
Construir a UI de Email sobre T-EML-01/02: caixa de entrada agrupada por thread (1 linha por
thread + contador), pastas com badge de não-lidos, seletor multi-conta com estado de sync por
conta + configurar conta IMAP/SMTP, thread com mensagens colapsáveis, **compositor em overlay
fullscreen** (exceção "editor precisa de mais área"), envio como saga (pendente→enviado; falha
nunca marca enviado) e **eco suprimido como estado visível** com explicação.

## 2. Contexto RAG (Spec-Driven Development)
- [mecanica-de-telas.md §B10](../docs/mecanica-de-telas.md) — mecânica validada no mockup (threading, saga de envio, eco suprimido, reentrega idempotente como tooltip, overlay do compositor). Decisão registrada: **sem** modal de confirmar envio — usar toast+Desfazer (§T3, padrão Gmail); confirmar no endurecimento.
- [mecanica-de-telas.md §T1/§T2](../docs/mecanica-de-telas.md) — assistente IA rascunha/resume thread (proposta); `email:message` é payload de drag (→ calendário cria evento com trecho citado; → ERP cria pendência).
- [inventario-de-telas.md §B10](../docs/inventario-de-telas.md) — composição exigida.
