...
- [ ] [M][M-017][validation-queue] Reentrância silenciosa em acquireValidationLease quando MGTIA_VALIDATION_QUEUE_ROOT vazio: `path.resolve(process.env.MGTIA_VALIDATION_QUEUE_ROOT ?? '')` cai em `path.resolve('')=process.cwd()`, que nunca é o `<git-common-dir>/mgtia-validation-queue`; comparação com `resolvedRoot` falha e o filho tenta acquire novo lease, criando deadlock mútuo com o pai. Latente (runQueuedCommand sempre seta ambos os env vars juntos), mas trivial de fixar — tools/scripts/lib/validation-queue.mjs:112. Fix de 1 linha: checar `MGTIA_VALIDATION_QUEUE_ROOT` antes de `path.resolve`. Discutido também em T-1025 (guard do TaskService).
