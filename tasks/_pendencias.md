...
- [ ] [i1][EST-13b][plugin-knowledge] **M1 do R1 RESOLVIDO pelo rework (commit `124bd6b`).** `package.json:7` mudou de `"./src/graph.ts"` para `"./src/index.ts"`; `dist/index.js` agora existe e re-exporta `makeGraph`+`makeFts`. Verificado em runtime: `node -e "import('./dist/index.js')..."` retorna `['makeFts', 'makeGraph']`. Wire contract com EST-14d View Knowledge agora funcional. ✅
