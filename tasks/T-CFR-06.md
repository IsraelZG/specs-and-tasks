---
id: T-CFR-06
title: "UI Contábil/Fiscal/RH (plano de contas, livro derivado, apuração/fechamento de competência, visão do contador, folha)"
status: draft:placeholder
complexity: 4
target_agent: frontend_agent # perfis: devops_agent, logic_agent, crypto_agent, frontend_agent
reviewer_agent: agile_reviewer
execution_mode: sequential # parallel | sequential
dependencies: ["T-CFR-01", "T-CFR-02", "T-CFR-03", "T-SHL-01", "T-PG-02"] # IDs de tarefas que bloqueiam esta
ui: true
...
# T-CFR-06 · UI Contábil/Fiscal/RH (plano de contas, livro derivado, apuração/fechamento de competência, visão do contador, folha)
...
## 1. Objetivo
Construir a UI Contábil/Fiscal/RH sobre T-CFR-01..03: plano de contas, livro de lançamentos
**derivados** (cada linha mostra a origem `derivedFrom`), apuração por competência com fechamento
destrutivo-com-aviso e **período fechado read-only de verdade** (tentar editar bloqueia com motivo
legível), visão do contador (subgrafo do cliente), folha/colaboradores, exportar SPED/NF-e.

## 2. Contexto RAG (Spec-Driven Development)
- [mecanica-de-telas.md §B4](../docs/mecanica-de-telas.md) — mecânica validada no mockup: bloqueio de edição em período fechado com `role="alert"` (nunca disable silencioso), DestructiveModal com label parametrizável ("Fechar competência"), recálculo retroativo que declara não alterar fechado, origem visível em todo lançamento.
- [mecanica-de-telas.md §T1](../docs/mecanica-de-telas.md) — assistente IA explica lançamento derivado e sugere conciliação (proposta).
- [inventario-de-telas.md §B4](../docs/inventario-de-telas.md) — composição exigida (estados: jurisdição ausente degrada, conector fiscal ausente — sem tela mockada, definir no endurecimento).
