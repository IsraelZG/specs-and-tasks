...
## 1. Objetivo
Resolver 2 achados não-bloqueantes acumulados em `packages/plugin-providers/`.

## 2. Contexto RAG (Spec-Driven Development)
- [ ] `packages/plugin-providers/src/fallback.ts` — circuit breaker + fallback
- [ ] `packages/plugin-providers/tests/telemetry.test.ts` — testes de telemetria
- [ ] `packages/plugin-providers/tests/registry.test.ts` — testes de registry

## 3. Escopo de Arquivos (Inputs e Outputs)
- **[UPDATE]** `packages/plugin-providers/src/fallback.ts` — implementar DEGRADED→CLOSED recovery
- **[UPDATE]** `packages/plugin-providers/tests/telemetry.test.ts` — adicionar assert para janela 60min
- **[UPDATE]** `packages/plugin-providers/tests/registry.test.ts` — fortalecer regex de teste 2
