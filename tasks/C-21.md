...
- **[UPDATE]** `packages/plugin-context/src/nanoPreprocess.ts` — remover `as any` cast e eslint-disables
- **[UPDATE]** `packages/plugin-context/src/optimize.ts` — importar `L2Tokenizer` em vez de redeclarar inline
- **[CREATE]** `packages/plugin-context/src/constants.ts` — extrair `TOK_EST` duplicada
