---
id: EST-35
title: "Implementar design Tactical Telemetry UI e css global"
...
**Diretrizes Cruciais do Design:**
1. **Tipografia:** Uso massivo de fontes monoespaçadas (Micro-Typography) para dados e telemetria (ex. `JetBrains Mono` ou `VT323`). Para Macro-Typography (Header), usar sans-serif pesada (ex. `Inter Black` em `clamp()`).
2. **Cores:** Fundo `#0A0A0A` a `#121212`, texto `#EAEAEA`. Acento exclusivamente vermelho (`#E61919`). Opcionalmente, um único elemento em `#4AF626` (Terminal Green) para status. NADA de gradientes ou drop-shadows suaves.
...
- `docs/tasks/EST-33.md` (Standalone build setup onde CSS foi omitido na migração Vite)

## 3. Escopo de Arquivos (Inputs e Outputs)
- **[NEW]** `apps/estaleiro/ui/src/index.css` (Definição de toda a arquitetura de styling bruta, CSS puro com variáveis CSS).
- **[UPDATE]** `apps/estaleiro/ui/src/main.tsx` (Adicionar `import "./index.css"`).
- **[UPDATE]** `apps/estaleiro/ui/src/views/**/*.tsx` (Ajustar classes existentes e injetar símbolos/borders ASCII/HTML semântico nos componentes de Board, Fleet, Execution, Decisions, etc.).
- **[UPDATE]** `apps/estaleiro/ui/index.html` (Inclusão de SVG filters para ruído/estática ou tags `<link>` para fontes do Google Fonts, como JetBrains Mono e Inter).
