---
id: T-AD-04
title: "UI de Anúncios (campanhas com pacing, criativos, segmentação, medição)"
status: draft:placeholder
complexity: 4
target_agent: frontend_agent # perfis: devops_agent, logic_agent, crypto_agent, frontend_agent
reviewer_agent: agile_reviewer
execution_mode: sequential # parallel | sequential
dependencies: ["T-AD-01", "T-AD-02", "T-SHL-01", "T-PG-02"] # IDs de tarefas que bloqueiam esta
ui: true
...
# T-AD-04 · UI de Anúncios (campanhas com pacing, criativos, segmentação, medição)
...
## 1. Objetivo
Construir a UI de Anúncios sobre T-AD-01/02: campanhas com barra de orçamento e **verba estourada
com precedência sobre o status** (pausa automática observável), criativos com preview no
vocabulário do PostCard (badge Patrocinado), segmentação declarativa com **`targetingBlocked` sem
vazar critérios**, dashboard de medição (CPM/CPC/CPA sobre cliques cobráveis, anti-fraude como
transparência) e "promover item existente" por referência.

## 2. Contexto RAG (Spec-Driven Development)
- [mecanica-de-telas.md §B9](../docs/mecanica-de-telas.md) — mecânica validada no mockup (verba estourada, segmentação parcial, cliques suspeitos excluídos da cobrança, promover sem duplicar).
- [mecanica-de-telas.md §T1/§T2](../docs/mecanica-de-telas.md) — Ads é **alvo** de drag (`marketplace:product`/`streaming:content` → campanha por referência); assistente IA gera criativo a partir do item promovido e sugere orçamento/segmentação (proposta).
- [inventario-de-telas.md §B9](../docs/inventario-de-telas.md) — composição exigida.
