---
id: T-MAP-04
title: "UI do Mapa (busca+lista+pins, detalhe de lugar, rota com proveniência, compartilhamento efêmero)"
status: draft:placeholder
complexity: 4
target_agent: frontend_agent # perfis: devops_agent, logic_agent, crypto_agent, frontend_agent
reviewer_agent: agile_reviewer
execution_mode: sequential # parallel | sequential
dependencies: ["T-MAP-01", "T-MAP-02", "T-SHL-01"] # IDs de tarefas que bloqueiam esta
ui: true
...
# T-MAP-04 · UI do Mapa (busca+lista+pins, detalhe de lugar, rota com proveniência, compartilhamento efêmero)
...
## 1. Objetivo
Construir a UI do Mapa sobre T-MAP-01/02: busca + lista ordenada por distância sincronizada com o
mapa de pins (render GeoSpatial com tiles reais — `maplibre-gl`), detalhe de lugar com salvar e
rota (sempre com **proveniência do conector** e ressalva de atualidade), compartilhamento de
localização **efêmero** (opt-in com TTL + revogar visível) e degradação parcial quando a
localização é negada (só o widget de posição degrada; lista/mapa seguem com cache).

## 2. Contexto RAG (Spec-Driven Development)
- [mecanica-de-telas.md §B5](../docs/mecanica-de-telas.md) — mecânica validada no mockup (lista↔pins, degradação parcial com retry, share com TTL de 15min + revogar, rota com proveniência).
- [mecanica-de-telas.md §T2](../docs/mecanica-de-telas.md) — `map:place` como payload de drag (→ calendário preenche local; → chat compartilha efêmero).
- [inventario-de-telas.md §B5](../docs/inventario-de-telas.md) + [diretrizes-ux.md §11](../docs/diretrizes-ux.md) — localização é dado sensível/efêmero.
