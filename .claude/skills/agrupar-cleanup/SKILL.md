...
5. **Rebuild + commit do controle:** o `generate-task.mjs` já regenera o INDEX. `git add tasks/ &&
   git commit -m "chore(cleanup): agrupa N pendências em C-NN (<áreas>)" && git push`.
