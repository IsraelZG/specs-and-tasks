...
3. **Atualiza manifesto de saúde (fire-and-forget).** Rode `node tools/scripts/saude.mjs` em background
   (não aguarde — o manifesto regenera assíncrono) para que o manifesto reflita o estado pós-merge.
4. **Verifica integração:** `node tools/scripts/drift-check.mjs` → sem novo integration drift para
   `$ARGUMENTS` (os arquivos do deliverable estão na master).
5. **Remove a worktree:** `node tools/scripts/worktree.mjs rm $ARGUMENTS` (preserva a branch).
