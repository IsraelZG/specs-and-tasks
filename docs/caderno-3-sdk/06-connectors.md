# 06-connectors.md — Conectores de Mundo Externo

Especifica a taxonomia completa de conectores externos da plataforma — toda integração com sistemas externos pertence a exatamente uma das cinco classes definidas aqui. Generaliza e substitui o escopo anterior (notificação out-of-band e push cego), que passa a ser §A (Classe A) e §B (Classe B) desta taxonomia.

---

## §0 — Taxonomia de Conectores Externos (RFC-007 A.1)
...
## §2 — Tradução API ↔ Grafo: Idempotência e Proveniência (RFC-007 A.3)
...
## §3 — Classe D: Espelho Bidirecional (RFC-007 A.4)
...
## §4 — Classe E: Provedor de Consulta (RFC-007 A.5)
...
## §5 — Registro de Instâncias Conhecidas (RFC-007 A.6)
...
## §A — Classe A: Egresso Notificacional (herdado)

*Conteúdo anterior do caderno — Interface `NotificationConnector` e casos de uso de SMTP, WhatsApp, SMS — permanece válido como Classe A da taxonomia.*

```typescript
interface NotificationConnector {
  send(recipient: Recipient, template: TemplateRef, payload: Record): Promise<void>;
  capabilities(): ConnectorCapabilities; // canais suportados, rate limits
}
```

Implementações: SMTP (base), Gmail API, WhatsApp Business API, SMS (Twilio).

---

## §B — Classe B: Ingresso Content-Blind (herdado)

*Conteúdo anterior do caderno — Push Connector para acordar dispositivos suspensos sem violar criptografia E2E.*

O Push Connector (VAPID/FCM/APNs) acorda dispositivos sem transmitir conteúdo: payload restrito a `blind_scope_id`, [[hlc]] do novo head e nonce anti-replay. Ver [[push-cego]] para limites honestos.
