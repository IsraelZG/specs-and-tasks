...
### Archetype: mcp-plugin

1. **mcp-plugin** é um archetype de plugin `node`, categoria `connector` (quando o servidor MCP bridge a serviço externo — ex.: repositório de código, base de dados, API de terceiro) ou `compute` (quando o servidor MCP prove função invocável local — ex.: transformação de dados, geração de código): expõe ferramentas e recursos ao agente de IA via protocolo MCP (Model Context Protocol), padronizando descoberta, invocação e resposta de tools. O protocolo é o transporte; a categoria depende da função que o servidor prove — `compute` para função invocável, `connector` para ponte ao mundo externo (§3).
2. **Contrato tipado por tool:** cada ferramenta exposta pelo servidor MCP é uma capacidade com schema de entrada/saída declarados, invocável por `ComputePort` (§4) — o agente descobre as tools disponíveis, invoca com parâmetros estruturados e recebe resposta normalizada, sem conhecer o backend. Determinismo segue a regra geral (§5.4): tools determinísticas admitem verificação por re-execução; não-determinísticas operam por confiança (assinatura + reputação).
3. **Classe de privacidade por tool:** cada ferramenta MCP declara sua classe de privacidade conforme os dados que manipula — tools que acessam repositórios privados ou credenciais do usuário são `restritas` (site `local`, nunca cruzam fronteira E2E sem consentimento explícito, §6.3); tools que operam sobre dado público podem ser elegíveis a site `peer`. O `ASSET:ROLE` efetivo do plugin invocado por um agente é a interseção com o `ASSET:ROLE` delegado ao agente (§6.4).
4. **Sessão e coordenação:** o estado de sessão de uma conversa MCP (contexto de tool-calling em andamento, streaming de resposta, coordenação multi-turno) reusa [[documento-casca]] + [[ephemeral-messages]]/Automerge (`REPLICABLE_VOLATILE`) — a mesma infra efêmera da plataforma, nenhum "session store" próprio. Trabalho durável (resultado persistido de tool-call) segue a fila assíncrona de §5.
5. **Distinção importante:** o archetype `mcp-plugin` é a capacidade de plataforma que qualquer agente pode invocar — não se confunde com os servidores MCP específicos que os agentes de desenvolvimento (MGTIA) usam como ferramenta de trabalho (ver `docs/playbook/06-ferramentas-lsp-mcp.md`). Aqueles são tooling do ambiente de construção; este é o contrato padronizado pelo qual a plataforma expõe tools a agentes em geral.

### Archetype: lsp-plugin

1. **lsp-plugin** é um archetype de plugin `node`, categoria `compute`: oferece capacidades de inteligência de linguagem (diagnósticos, referências, autocomplete, navegação de símbolos, formatação) via protocolo LSP (Language Server Protocol), padronizando a interação entre o agente/editor e o servidor de linguagem. O protocolo é o transporte; a categoria é `compute` porque o servidor prove função invocável (análise de código) — não bridge a serviço externo.
2. **Sandbox de projeto:** o plugin roda em isolate com acesso restrito ao diretório de projeto declarado como workspace do servidor de linguagem — acesso a grafo, rede e FS só pelas portas concedidas pelo `ASSET:ROLE` (§6.1). Semelhante ao fs-plugin neste aspecto, mas escopado a operações de análise de código (não execução de comandos arbitrários).
3. **Classe de privacidade restrita:** o LSP acessa código-fonte do usuário, que é dado sensível do workspace local. Classificado como `restrito` conforme [[classe-de-privacidade]] (§6.3) — execução fixada a site `local`, nunca cruza fronteira E2E para `peer` ou `external` sem consentimento explícito.
4. **Distinção importante:** o archetype `lsp-plugin` é a capacidade de plataforma que qualquer agente pode invocar para inteligência de linguagem — não se confunde com os servidores LSP específicos que os agentes de desenvolvimento (MGTIA) usam como ferramenta de trabalho (ver `docs/playbook/06-ferramentas-lsp-mcp.md`). Aqueles são tooling do ambiente de construção; este é o contrato padronizado pelo qual a plataforma expõe inteligência de linguagem a agentes em geral.
