...
| 020-03 | Visão Diária Condensada: `CalendarGrid` colapsa visualmente horas ociosas (Time-Blocking UI) (§3) | UI->INVENTARIO | inventario-componentes-layouts.md | Layout `CalendarGridCondensed` (organismo, módulo Calendário) — grid diário que colapsa faixas horárias ociosas para maximizar blocos produtivos. | [ ] |
| 020-04 | Atom `EventColorPill` (§3) | UI->INVENTARIO | inventario-componentes-layouts.md | `EventColorPill` (átomo, módulo Calendário) — pílula de cor de categorização de evento. | [ ] |
| 020-05 | Atom `RSVPIconButton` (§3) | UI->INVENTARIO | inventario-componentes-layouts.md | `RSVPIconButton` (átomo, módulo Calendário) — botão de ação de RSVP (aceito/recusado/talvez). | [ ] |
| 020-06 | Molecules `AgendaDayBlock` e `AvailabilitySlot`; Organisms `TimelineGridRenderer` e `EventDetailsSidebar` (§3) | UI->INVENTARIO | inventario-componentes-layouts.md | `AgendaDayBlock` (molécula), `AvailabilitySlot` (molécula), `TimelineGridRenderer` (organismo — drag/resize no eixo Y), `EventDetailsSidebar` (organismo) — módulo Calendário. | [ ] |
