...
- [[approves]] — Aresta de `PROFILE:SYSTEM` → `CONTENT:INTENT`; aprovação de validador com evidência inline individualizada. Dispara a finalização pelo [[aplicador-deterministico]].
...
- [[vfk]] — Constraint de integridade referencial condicionada aplicada em $O(1)$ na camada de aplicação, usando o **11º caractere (index 10)** do ULID (`N` = nodes, qualquer outro = edges) para determinar a tabela-alvo da aresta.
...
- [[caderno-5-transport/01-p2p-transport-and-reconciliation#241-os-três-estados|Bootstrap Frio Absoluto]] — Primeiro login da identidade num device numa rede: sem grafo, sem histórico de peers. Só entra por canal out‑of‑band (mDNS, link multiaddr, convite, URL do peer do sistema). Ver RFC §2.4.1.
- [[caderno-5-transport/01-p2p-transport-and-reconciliation#241-os-três-estados|Bootstrap Morno (Resume)]] — Reabrir o app após desconexão: há grafo + histórico de peers; refresca coordenadas e cai na Onda 0. A meta de latência < 500 ms é alcançável neste estado. Ver RFC §2.4.1.
- [[caderno-5-transport/01-p2p-transport-and-reconciliation#242-canais-outofband-primeira-conexão-cold-start|Link Multiaddr]] — Artefato de conectividade pura (endereçamento libp2p), compartilhável e reutilizável; não concede identidade nem acesso. Distinto do convite. Ver RFC §2.4.4.
...
- [[defesa-sybil]] — Conjunto de mecanismos (convite gateado por standing, staking social, irrelevância por diversidade, bond/caução, detecção topológica) que torna Sybils irrelevantes. Defesa primária = custo de identidade, separada da economia. P2P puro: opt-in via SPEC; redes gerenciadas: resolvido na porta. Ver rfc-v4.md §4.2.
- [[bond-caucao]] — Depósito de valor exigido de agentes em papéis privilegiados (validador, custódio, oráculo); mau comportamento verificável corta a caução. Único ponto em que a economia tem papel de segurança legítimo. Opt-in via SPEC. Ver rfc-v4.md §4.2 e rfc-transacoes-multidominio.md §6.3.
