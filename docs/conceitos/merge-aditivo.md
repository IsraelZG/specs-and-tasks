...
## Definição
O **Merge Aditivo** (ou **Merge Aditivo de Saldo**) é a regra de resolução de conflitos e reconciliação da Plataforma Projeto SuperApp V0.41 para operações comutativas de crédito sobre a linhagem de um ativo (`ASSET:BALANCE_STATE`). Quando transações de crédito concorrentes bifurcam a linhagem do ativo de destino, o sistema mescla as ramificações somando deterministicamente os deltas das operações (`base + Δ₁ + Δ₂`), evitando o uso de estratégias do tipo Last-Write-Wins (LWW), que causariam a perda de valores financeiros e de histórico.
