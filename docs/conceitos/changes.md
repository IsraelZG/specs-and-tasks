...
- **Captura em Staging**: As alterações geradas na UI local em tempo real são salvas na tabela local não-replicada [[pending-changes]] (ver [[caderno-2-protocol/04-automerge-integration-spec#31-captura-de-changes-escrita-em-staging]]).
...
- **Emissão e Limpeza**: Ao disparar o gatilho, o committer gera o snapshot binário consolidado (`Automerge.save(doc)`), persiste o nó-versão no grafo (`nodes`) com as arestas `MUTATES` e `AUTHORED` correspondentes, e limpa as `changes` consolidadas da tabela local [[pending-changes]]. O ciclo completo de coalescência + consolidação + trilha de auditoria semântica é denominado [[mfa-s|MFA-S]].

## Implementação

No SDK da plataforma, o controle do ciclo de vida e a persistência em memória/SQLite das `changes` são descritos em:
- **[[caderno-3-sdk/02-sync-worker-and-memory-lifecycle#4-garbage-collection-híbrido-g4-e-quotas]]**: Onde é detalhado como o Garbage Collector (G4) compacta as `changes` brutas da tabela local e consolida o log do [[automerge-repo]].
- **[[caderno-3-sdk/01-sqlite-and-projections-schema#4-matriz-de-classificação-de-transporte-as-3-perguntas]]**: Mapeamento de `pending_changes` na categoria `REPLICABLE_VOLATILE` do banco SQLite local (`sqlite_pending_changes`) com protocolo `EPHEMERAL_WEBRTC`.
