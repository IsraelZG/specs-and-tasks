# RFC-008 — Linguagem de Páginas (UI Spec-Based)
> **Status:** Proposta
> **Precedência:** estende `caderno-3-sdk/03-engines-and-spec-driven-ui.md` §3 (`ui_hints`) e RFC-006 (catálogo, tokens, metadados). Onde não tocada, a doc vigente prevalece. Pré-requisito de **todas** as RFCs de produto (011+): módulos canônicos são autorados nesta linguagem.
> **Decisões de entrada:** expressões exclusivamente ZEN (motor único de lógica); dialeto próprio (padrões de mercado absorvidos, formato nosso); formulários no padrão JSON Forms (dataschema/uischema).

## A.1 — Página como nó do grafo

**Resolve:** onde vive uma página e por que isso entrega customização sem recompilar e criação on-the-fly (inclusive por IA) de graça.
...
## A.2 — O documento: árvore restrita ao catálogo

**Resolve:** a estrutura do dialeto e os parâmetros fixos que eliminam margem de abuso.
...
## A.3 — Dados: fontes declaradas, expressões ZEN puras

**Resolve:** como a página acessa dados sem ganhar capacidade de I/O.
...
## A.4 — Lógica complexa mora no catálogo, não na página

**Resolve:** como ter planilha com fórmulas, player de vídeo, editor rico — sem abrir JavaScript na linguagem.
...
## A.5 — Ações: o teto de abuso

**Resolve:** o que uma página pode *fazer*.
...
## A.6 — Formulários (padrão JSON Forms)

**Resolve:** o caso form-heavy (ERP é majoritariamente formulário) com reuso máximo.
...
## A.7 — Validação, confiança e geração por IA

**Resolve:** o ciclo de vida de publicação e o caso Lovable.
...
| Arquivo | Seção | Ação |
| :--- | :--- | :--- |
| `docs/plano-de-implementacao.md` | preparativos de módulos | Adicionar T-PG-01..05 |

**T-PG-01** schema JSON do dialeto v1 + validador estático (lib isomórfica, DoD Protocolo/core); **T-PG-02** renderizador React sobre o catálogo (resolve `sources` via TinyBase, avalia ZEN sob orçamento, render progressivo); **T-PG-03** mecanismo `EXTENDS`/override por id estável + testes de precedência; **T-PG-04** sub-dialeto de formulários sobre dataschema de SPEC; **T-PG-05** vetores adversariais: página com componente fora do catálogo, expressão estourando orçamento, intent acima do privilégio do autor, árvore acima dos limites — todos com rejeição comprovada (doutrina de honestidade radical §0.1.7).
