...
8. **Fecha o status pelo serviço:** o ator é `agile_reviewer:<SeuModelo>` (papel autoriza, modelo fica
   no ledger — ver "Identidade do agente" no AGENTS.md): `node tools/scripts/manage-task.mjs approve
...
9. **Persiste o controle — ENFILEIRE** (agentes não rodam git no Docs; ver Paralelismo no AGENTS.md).
