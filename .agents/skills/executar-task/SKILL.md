...
Use seu identificador `Antigravity` onde aparecer `<EU>` — **nunca** `agile_reviewer`.
...
3. **Inicie** (ledger no controle): `node "<CTRL>/tools/scripts/manage-task.mjs" start $ARGUMENTS Antigravity "iniciando"`.
...
6. **Finalize** (ledger): `node "<CTRL>/tools/scripts/manage-task.mjs" finish $ARGUMENTS Antigravity "<resumo + placar de testes>"`.
